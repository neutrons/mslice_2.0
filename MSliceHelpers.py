...
def plot1Dplot(ws,compFilename):
    ...
    """
    ion forces matplotlib to use its own threads which avoids having the
...
#    plt.subplots_adjust(left=0.2,right=0.9)
    ...
    """
    #attempts here to control the tick font - was able to change some params but not the fontname eventhough it was explicitly set...
    for item in ([ax.xaxis.label, ax.yaxis.label] +
             ax.get_xticklabels() + ax.get_yticklabels()):
        item.set_fontsize(14)    
        item.set_fontname('Courier')
    for tick in ax.xaxis.get_major_ticks():
        #tick.label.set_fontsize(34) 
        tick.label.set_fontname("Courier")
    """
    
    titleStr="1D Plot: "+ws.name()
    titleStr=titleStr.replace('_','\_') #need to embed as \_ to supress subscripting
    ax.set_title(titleStr,fontdict=fontdict)

    #place x/y grid on the plot
    ax.grid(True)

    #xoff used for left/right separation between text blocks
    xoff0=-0.01
    xoff1=0.55
    #yoff used for global offset up/down
    yoff=-0.25
    #dx0,dy0 - lower left point
    dx0=-0.05
    dx1=dx0
    #dx1,dy1 - upper right point
    dy0=0
    dy1=-0.06
    ...
    #place HKLE info
    plt.annotate(r'\underline{HKLE Info}',(dx0+xoff1,dy0*0),(dx1+xoff1,dy1*4+yoff),textcoords='axes fraction')    
    ...
    #note that plt.tight_layout() will kill the adjust formatting 
    
    plt.gca().set_autoscale_on(False)
    
    plt.draw() 
    #note that plt.show() will show all figures, thus plt.draw() is needed here
